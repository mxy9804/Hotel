...
<script type="text/javascript">
	function editInfo(obj,flag){
		if(flag==0){
			...
			$("#editForm").attr("action","<%=basePath%>addstaff.do");
			
		} else {
			$("#editForm").attr("action","<%=basePath%>modifystaff.do");
			var staffInfo = obj.parentNode.parentNode.childNodes;
			console.log(staffInfo);
			$("#staffId").val(staffInfo[1].innerHTML);
			$("#staffName").val(staffInfo[3].innerHTML);
			$("#staffAge").val(staffInfo[5].innerHTML);
			$("#staffType").val(staffInfo[7].innerHTML);
			$("#staffTele").val(staffInfo[9].innerHTML);
			$("#staffCardNumber").val(staffInfo[11].innerHTML);
			$("#staffAdress").val(staffInfo[13].innerHTML);
			$("#staffStatus").val(staffInfo[15].innerHTML);
		}
	}
	
	fuction doDelete(staffId){
		if(confirm("您确定要删除编号为："+staffId+"的商品吗")){
			location.href="<%=basePath%>removestaff.do?staffId=" + staffId;
		}
	}
